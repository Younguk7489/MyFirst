...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSoomcoinAmount"))
        settings.setValue("nAnonymizeSoomcoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSoomcoinAmount = settings.value("nAnonymizeSoomcoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSoomcoinAmount"))
        SoftSetArg("-anonymizeSoomcoinamount", settings.value("nAnonymizeSoomcoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSoomcoinAmount:
            nAnonymizeSoomcoinAmount = value.toInt();
            settings.setValue("nAnonymizeSoomcoinAmount", nAnonymizeSoomcoinAmount);
            emit anonymizeSoomcoinAmountChanged(nAnonymizeSoomcoinAmount);
            break;
        default:
            break;
        }
