...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
  ...
  if (!TestNet())
    {
      if ( nHeight < ONE_YEAR_BLOCKS * 3)
          nSubsidy = 20 * COIN;
      else if(nHeight >= ONE_YEAR_BLOCKS * 4 && nHeight < ONE_YEAR_BLOCKS * 5)
          nSubsidy = 10 * COIN;
      else if(nHeight >= ONE_YEAR_BLOCKS * 5)
          nSubsidy = 5 * COIN;
    }
  else
    {
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    ...
    if( nHeight > 4 * ONE_YEAR_BLOCKS && nHeight <= 5 * ONE_YEAR_BLOCKS)
        nSubsidy = 4 * COIN_YEAR_REWARD * nCoinAge *33 / nDiv;
    else if( nHeight > 5 * ONE_YEAR_BLOCKS && nHeight <= 6 * ONE_YEAR_BLOCKS)
        nSubsidy = 3 * COIN_YEAR_REWARD * nCoinAge * 33 / nDiv;
    else if ( nHeight > 6 * ONE_YEAR_BLOCKS && nHeight <= 7 * ONE_YEAR_BLOCKS)
        nSubsidy = 2 * COIN_YEAR_REWARD * nCoinAge * 33 / nDiv;
    else if ( nHeight > 7 * ONE_YEAR_BLOCKS )
...
#ifdef ENABLE_WALLET
// soomcoin: attempt to generate suitable proof-of-stake
